...
void CBinds::SetDefaults()
{
	...
	Bind(KEY_RIGHTARROW, "+right");
	Bind(KEY_LEFTARROW, "+left");
	Bind(KEY_UPARROW, "+jump");
	Bind(KEY_DOWN, "+hook");
	...
	Bind(KEY_LEFTARROW, "spectate_previous");
...
void CBinds::SetDDRaceBinds(bool FreeOnly)
{
	...
	else
	{
		...
		if(!Get(KEY_PAUSE)[0])
			Bind(KEY_PAUSE, "say /pause");
		if(!Get(KEY_UPARROW)[0])
			Bind(KEY_UPARROW, "+jump");
		if(!Get(KEY_LEFTARROW)[0])
			Bind(KEY_LEFTARROW, "+left");
		if(!Get(KEY_RIGHTARROW)[0])
			Bind(KEY_RIGHTARROW, "+right");
		if(!Get('[')[0])
			Bind('[', "+prevweapon");
		if(!Get(']')[0])
